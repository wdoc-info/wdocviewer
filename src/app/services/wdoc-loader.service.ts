...
export class WdocLoaderService {
  ...
  async fetchAndLoadWdoc(url: string): Promise<WdocLoadResult | null> {
    try {
      ...
      await this.dialogService.alert('Error downloading .wdoc/.zip file from URL.');
      return null;
    }
  }
  ...
  async loadWdocFromArrayBuffer(
    ...
    try {
      ...
      if (!indexFile) {
        await this.dialogService.alert('index.html not found in the archive.');
        return null;
      }
      ...
      await this.dialogService.alert('Error processing .wdoc file.');
      return null;
    }
  }
